...
def generate_backstory(backstory_idx: int, cfg: dict, tokenizer):
    ...
    if preamble_question:
        ...
        while True:
            ...
            if "yes" in preamble_answer_result.text.lower():
                ...
                break
        backstory_question = f"\n\n{surveyor}{connector} {backstory_question} Please describe in detail.\n\n{respondent}{connector}"
    ...
    backstory_length_failcount = 0
    llm_testing_failcount = 0
    ...
    while True:
        ...
        generated_backstory_result = [
            backstory
            for backstory in generated_backstory_result
            if len(backstory.text.split()) >= 250
        ]

        # check if "interview" is mentioned more than 2 times
        generated_backstory_result = [
            backstory
            for backstory in generated_backstory_result
            if len(re.findall("interview", backstory.text, re.IGNORECASE)) <= 1
        ]
        if generated_backstory_result:  # minimum wordcount
            final_backstory_candidates = []
            for backstory_candidate in generated_backstory_result:
                question_prompt = "Is the given text strictly a first-person narrative of an individual? Provide a brief reason and answer with yes or no. If the text mentions an interview, contains multiple questions and answers, or mentions that it is a blog post, make sure to answer No.\n"
                user_prompt = (
                    "[Text 1]"
                    + ' This is a typical interview question to assess your ability to think out loud. Here is a sample answer: "I am 30 years old, grew up in Georgia. I am always bright and clever, always willing to help a person in need.'
                    + "\n\n"
                    + "[Question 1]"
                    + question_prompt
                    + "[Answer 1] Because it is not written in the first-person narrative and its topic is an interview, the answer is no.\n\n"
                    + "[Text 2]"
                    + " My personality of always being willing to go above and beyond is the thing that makes me who I am. In my personal and professional relationships, I'm known as a go-getter. I'm not someone who will come in and say the bare minimum."
                    + "\n\n"
                    + "[Question 2]"
                    + question_prompt
                    + "[Answer 2] Because all parts of the text are written in a first-person narrative and it is a story that is not related to an interview, the answer is yes.\n\n"
                    + "[Text 3]"
                    + " I am a 18-year old boy who is very passionate about technology.\n\n\"Why are you passionate about technology?\"\n\nI am passionate about technology because I believe it has the power to change the world for the better.'\n\nWhat do you hope to achieve in the future?\n\nI don't think I am going to be a super rich."
                    + "\n\n"
                    + "[Question 3]"
                    + question_prompt
                    + "[Answer 3] Because it takes an interview format of questions and answers, the answer is no.\n\n"
                    + "[Text 4]"
                    + backstory_candidate.text
                    + "\n\n"
                    + "[Question 4]"
                    + question_prompt
                    + "[Answer 4]"
                )
                llm_classification_response = prompt_llm_chat(
                    system_prompt="Your job is to classify whether the following text is a story of a person that is written in the first-person narrative.",
                    user_prompt=user_prompt,
                    max_tokens=128,
                    model_name="gpt-4o",
                )
                if re.findall(
                    "The answer is yes", llm_classification_response, re.IGNORECASE
                ):
                    final_backstory_candidates.append(backstory_candidate)
                    break
                else:
                    llm_testing_failcount += 1
            if final_backstory_candidates:
                break
        else:
            backstory_length_failcount += 1
    generated_backstory_result = final_backstory_candidates[0]
    ...
    if cfg.debug:
        log.info(f"Failed backstory due to length: {backstory_length_failcount}")
        log.info(f"Failed backstory due to LLM classification: {llm_testing_failcount}")
